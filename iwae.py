...
# def save_mean_q_variances(model, dataset, srng, directory=None):
#     x, _ = dataset.minbatchIindex_minibatch_size(0, 500, subdataset='train', srng=srng)

#     samples = model.samplesIx_srng(x, srng)

#     means = []
#     sigmas = []
#     for layer, x in zip(model.q_layers, samples):
#         mean, sigma = layer.mean_sigmaIx(x)
#         means.append(mean)
#         sigmas.append(sigma)

#     mean_sigma_fun = theano.function([], means+sigmas)
    
#     mean_sigma_vals = mean_sigma_fun()
#     mean_vals = mean_sigma_vals[:len(mean_sigma_vals)/2]
#     sigma_vals = mean_sigma_vals[len(mean_sigma_vals)/2:]

#     stds_of_means = [np.std(mean_val, axis=0) for mean_val in mean_vals]
#     vars_of_means = [np.var(mean_val, axis=0) for mean_val in mean_vals]
#     means_of_means = [np.mean(mean_val, axis=0) for mean_val in mean_vals]
#     stds_ords = [np.argsort(stds_mean)[::-1] for stds_mean in stds_of_means]

#     stds_of_sigmas = [np.std(sigma_val, axis=0) for sigma_val in sigma_vals]
#     means_of_sigmas = [np.mean(sigma_val, axis=0) for sigma_val in sigma_vals]

#     stds_of_means_ordered = [std_mean[std_ord] for std_mean, std_ord in zip(stds_of_means, stds_ords)]
#     vars_of_means_ordered = [var_mean[std_ord] for var_mean, std_ord in zip(vars_of_means, stds_ords)]
#     means_of_means_ordered = [mean_mean[std_ord] for mean_mean, std_ord in zip(means_of_means, stds_ords)]
#     stds_of_sigmas_ordered = [std_sigma[std_ord] for std_sigma, std_ord in zip(stds_of_sigmas, stds_ords)]
#     means_of_sigmas_ordered = [mean_sigma[std_ord] for mean_sigma, std_ord in zip(means_of_sigmas, stds_ords)]

#     vars_of_means_ordered_trunc = [var_mean[std_ord][20:50] for var_mean, std_ord in zip(vars_of_means, stds_ords)]
    
#     import matplotlib
#     matplotlib.use('Agg')
#     import matplotlib.pyplot as plt
#     import os
#     for i, var_mean in enumerate(vars_of_means_ordered):
#         plt.hist(np.log(var_mean), 30)
#         plt.xlabel('$\log$ variance of $\mu$')
#         plt.ylabel('number of units')
#         plt.savefig(os.path.join(directory, 'log_stds_means_hist{}.pdf'.format(i)))
#         plt.close()

#     return logger.MultipleGraphValues(
#         stds_of_means_ordered + vars_of_means_ordered + vars_of_means_ordered_trunc + means_of_means_ordered + stds_of_sigmas_ordered + means_of_sigmas_ordered,
#         ['mean_q_stds_{}'.format(i) for i in range(len(stds_of_means_ordered))] +
#         ['mean_q_vars_{}'.format(i) for i in range(len(vars_of_means_ordered))] +
#         ['mean_q_vars_trunc_{}'.format(i) for i in range(len(vars_of_means_ordered_trunc))] +
#         ['mean_q_means_{}'.format(i) for i in range(len(means_of_means_ordered))] +
#         ['sigma_q_stds_{}'.format(i) for i in range(len(stds_of_sigmas_ordered))] +
#         ['sigma_q_means_{}'.format(i) for i in range(len(means_of_sigmas_ordered))])
...
# def chop_units(model, units):
#     num_units_in_input = model.q_layers[0].h_network.layers[0].W.get_value().shape[0]
#     for q_layer, p_layer, units_in, units_out in zip(model.q_layers, reversed(model.p_layers), [num_units_in_input]+units, units):
#         mean_net = q_layer.mean_network
#         sigma_net = q_layer.sigma_network
#         h_net = q_layer.h_network
#         mean_net.W.set_value(mean_net.W.get_value()[:, :units_out])
#         mean_net.b.set_value(mean_net.b.get_value()[:, :units_out])
#         sigma_net.layers[0].W.set_value(sigma_net.layers[0].W.get_value()[:, :units_out])
#         sigma_net.layers[0].b.set_value(sigma_net.layers[0].b.get_value()[:, :units_out])
#         h_net.layers[0].W.set_value(h_net.layers[0].W.get_value()[:units_in, :])

#         if isinstance(p_layer, BernoulliSampler):
#             mean_net = p_layer.mean_network
#             mean_net.layers[0].W.set_value(mean_net.layers[0].W.get_value()[:units_out, :])
#         elif isinstance(p_layer, GaussianSampler):
#             mean_net = p_layer.mean_network
#             sigma_net = p_layer.sigma_network
#             h_net = p_layer.h_network
#             mean_net.W.set_value(mean_net.W.get_value()[:, :units_in])
#             mean_net.b.set_value(mean_net.b.get_value()[:, :units_in])
#             sigma_net.layers[0].W.set_value(sigma_net.layers[0].W.get_value()[:, :units_in])
#             sigma_net.layers[0].b.set_value(sigma_net.layers[0].b.get_value()[:, :units_in])
#             h_net.layers[0].W.set_value(h_net.layers[0].W.get_value()[:units_out, :])

#     return model,

# def mean_pIinpt_level(model, inpt, level, srng):
#         x = inpt
#         for layer in model.p_layers[level:-1]:
#             x = layer.samplesIx_srng(x, srng)
#         return model.p_layers[-1].meanIx(x)

# def get_reconstructions_same_forward(model, dataset, srng, num_repeats=20, directory=None):
#     x, _ = dataset.minbatchIindex_minibatch_size(0, 20, subdataset='train', srng=srng)
#     samples = model.samplesIx_srng(x, srng)[1:]
#     samples = [T.repeat(s, num_repeats, axis=0) for s in samples]
#     reconstructions = [mean_pIinpt_level(model, sample, len(model.p_layers)-i-1, srng) for i, sample in enumerate(samples)][1:]

#     reconstructions_fun = theano.function([], reconstructions)
#     reconstructions_vals = reconstructions_fun()
#     reconstructions_vals_reshaped = [reshape_array(rec) for rec in reconstructions_vals]
#     return logger.MultipleImageValues([reshape_array(T.repeat(x, num_repeats, axis=0).eval())]+reconstructions_vals_reshaped,
#         ["originals"] + ["recs_same_forward_{}".format(i+2) for i in range(len(reconstructions_vals_reshaped))])

# def get_reconstructions_diff_forward(model, dataset, srng, num_repeats=20, directory=None):
#     x, _ = dataset.minbatchIindex_minibatch_size(0, 20, subdataset='train', srng=srng)
#     samples = model.samplesIx_srng(T.repeat(x, num_repeats, axis=0), srng)[1:]
#     reconstructions = [mean_pIinpt_level(model, sample, len(model.p_layers)-i-1, srng) for i, sample in enumerate(samples)]

#     reconstructions_fun = theano.function([], reconstructions)
#     reconstructions_vals = reconstructions_fun()
#     reconstructions_vals_reshaped = [reshape_array(rec) for rec in reconstructions_vals]
#     return logger.MultipleImageValues([reshape_array(T.repeat(x, num_repeats, axis=0).eval())]+reconstructions_vals_reshaped,
#         ["originals"] + ["recs_diff_forward_{}".format(i+1) for i in range(len(reconstructions_vals_reshaped))])